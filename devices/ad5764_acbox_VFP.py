...
class ad5764_acbox_VFP_widget(gui.QWidget):
    def __init__(self,parent,connection,com):
        ...
        self.input_frq  = floatInput(self,[0,1e8],4,'',[sp_x * 2 + ll, ls*1, ll, ls],"set frequency")#Changed this to float from int, added 4 for sig figs-Zach
...
(X2 full scale / X1 full scale) * (X2 value / X1 value) * cos(phase)""" 
    ...
    def write_frq(self):
        ...
        if not (str(value) == 'nan'):
            self.connection.ad5764_acbox.select_device(self.device)
            print(self.connection.ad5764_acbox.set_frequency(self.input_frq.getValue()))#removed float(self.input...) conversion-Zach
    ...
    def update_readouts(self,voltages):
        for entry in voltages:
            if "ad5764_acbox (%s)"%entry[0] == self.com:
                for port in range(4):
